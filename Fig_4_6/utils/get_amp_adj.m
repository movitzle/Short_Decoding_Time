function [amps_adj] = get_amp_adj(D, N, loc, xi, width, mu, type)
    ...
    n_subgroups = max(size(xi)); % number of modules
    n_sub_pop_points = N/n_subgroups; % neurons per module
    xi_vec = repmat(xi, 1, n_sub_pop_points); % indivdual ord. freq. per neruon
