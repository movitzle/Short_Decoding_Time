function [MSE, r_cell, T, amps, approx_FI, est_FI, sq_errs_cell, s_est_cell, s_true_cell] = find_Tth(alpha, D, mu, T_vec, loc, width, xi, background_activity, N, s_fixed, n_starting_points, s_FI)
    ...
    % For different values of decoding time T, check if minimal decoding
    % time is met. If so, break out of the for-loop
    for j=1:length(T_vec)
        ...
        % A bit unnecessary to redefine the population every time
        % step just to adjust the time (i.e., scale the rates)
        ...
        if mean(MSE(:,j)) < alpha*mean(diag(inv(T*est_FI)))
            break; % If average MSE is within the range - exit and save T as minimal decoding time.
        end
    end
    r_cell = r;
