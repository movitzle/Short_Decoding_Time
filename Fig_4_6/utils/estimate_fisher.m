function [mean_FI] = estimate_fisher(D, f, q, q_i, grad_q_i, amps, s_true)
    ...
    % Estimate DxD fisher matrix using n_stim stimulus points
    fisher_mat_per_stim = zeros(D,D,n_stim);
    for j=1:n_stim
        s = s_true(:,j);
        for k=1:D
            for l=1:D
                if l==k %Diagonal elements
    ...
    mean_fisher_mat = mean(fisher_mat_per_stim,3); %Average across the stimulus conditions
